...
- O ambiente local possui carga de demonstração reproduzível com administrador, dois bibliotecários, estudantes nas principais etapas do fluxo e Magic Link da coordenação. Senhas, CPFs completos e token são gerados em execução, gravados somente em `.auth/mock-users.txt` e nunca versionados.
