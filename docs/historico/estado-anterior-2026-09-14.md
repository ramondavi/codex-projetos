Registro histórico arquivado em 14/09/2026. Contagens e implantação abaixo não são verificações atuais. Consultar somente por necessidade histórica.
...
- Refinamento de interface em implementação local: identidade institucional e rodapés responsivos, conta destacada no cabeçalho, navegação rápida por perfil e FAQ administrável. O calendário operacional foi concluído localmente: Administração com navegação mensal/anual, feriados e pontos facultativos federais de 2026, cálculo de SLA, registro manual de indisponibilidade e painel estudantil restrito aos eventos do mês corrente.
...
20. `202608310001_staff_provisioning_notifications.sql`.
21. `202608310002_privacy_notice_acknowledgements.sql`.
22. `202608310003_grant_privacy_acknowledgement_read.sql`.
23. `202608310004_request_analysis_completion_and_reset.sql`.
24. `202608310005_require_student_cataloging_details.sql`.

As 14 primeiras migrações estão aplicadas no projeto remoto `Pronto!`. As migrações 15 a 24 estão aplicadas somente no Supabase local. Nenhuma delas será enviada ao remoto sem revisão de impacto e confirmação explícita.
