...
- Templates e justificativas aparecem somente para campos devolvidos. Correções diretas feitas pelo bibliotecário responsável são auditadas e exibem o valor anterior como informação corrigida e sem validade. Durante a análise em edição, o responsável pode restaurar apenas as correções diretas registradas naquela revisão aos valores enviados pelo estudante; o estorno também é auditado e nunca apaga o histórico.
...
- Para economizar tokens, alterações visuais acumuladas em uma mesma branch recebem uma única validação visual consolidada antes do PR, somente após aprovação do usuário. Excepcionalmente, o agente deve justificar e obter aprovação antes de validar uma alteração relevante durante o desenvolvimento, quando isso for necessário para prosseguir com segurança.
...
- Antes de uma ação de alto consumo de tokens, informar finalidade, estimativa e impacto percentual na cota quando esse dado estiver disponível, e obter aprovação. A regressão completa permanece para a etapa de PR, também mediante aprovação.
