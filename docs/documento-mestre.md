...
- O idioma da interface pertence à aplicação inteira, pública e autenticada. Na primeira visita, usa o idioma preferido do dispositivo quando disponível entre português, inglês, espanhol, alemão, francês e italiano; nos demais casos, português. A escolha explícita pelo menu com globo no rodapé persiste e prevalece sobre a detecção. Quando o idioma do dispositivo divergir da escolha, o rodapé oferece uma sugestão discreta e opcional para alternar ou manter. O formulário não tem seletor de idioma próprio.
- Cada linha de palavras-chave representa um par: idioma original à esquerda; português à direita para trabalhos estrangeiros, ou inglês à direita para trabalhos originalmente em português.
- A revisão mostra os valores preenchidos e destaca pendências. Apenas itens com erro ou pendência levam diretamente ao campo, que recebe foco e destaque temporário. O botão de envio permanece desativado até que todas as validações do formulário sejam atendidas, com uma explicação visível.
...
- Alterações visuais acumuladas recebem validação consolidada antes do PR, executada pelo agente com aplicação em funcionamento e captura. Inspeções intermediárias são feitas quando necessárias para corrigir o escopo; não dependem de aprovação técnica do usuário. Testes usam dados sintéticos e ambiente local, sem operações em produção.
- Durante o desenvolvimento, preferir inspeções pontuais, revisão do diff e testes direcionados; evitar reexibir arquivos, saídas, logs ou capturas sem necessidade e não executar análises paralelas apenas por garantia.
- O agente executa e corrige autonomamente verificações locais e a regressão necessária ao PR, sem transferir comandos, revisão de código ou diagnóstico ao usuário não-programador. Usa npm run verify para resumir test, typecheck, lint e build, preserva logs e reexecuta somente o afetado após correções. Mantém E2E e pgTAP do CI. Login, decisões institucionais, migrações remotas e ações destrutivas continuam sujeitos às autorizações aplicáveis; economia não elimina testes nem fundamenta promessas de percentual de cota.
