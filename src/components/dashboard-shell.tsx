    ...
    <div className="dashboard-shell dashboard-shell--compact">
      <aside className={`dashboard-nav${expanded ? "" : " dashboard-nav--compact"}`} onMouseEnter={() => setExpanded(true)} onMouseLeave={() => setExpanded(false)} onFocusCapture={() => setExpanded(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setExpanded(false); }}>
        <div className="dashboard-nav__top">
          <Link href="/" className={expanded ? "dashboard-nav__brand-logo" : "dashboard-nav__favicon"} aria-label={t.public} title={t.public}>{expanded ? <><Image className="brand__image brand__image--light" src="/logo-pronto-light.png" alt="Pronto!" width={1600} height={643} priority /><Image className="brand__image brand__image--dark" src="/logo-pronto-dark.png" alt="Pronto!" width={1600} height={643} priority /></> : <Image src="/icon.png" alt="Pronto!" width={64} height={64} priority />}</Link>
        </div>
        {isStaff ? <nav aria-label={t.library}>
          ...
          {role === "administrator" && <div className="dashboard-nav__admin"><button className={`dashboard-nav__admin-trigger${pathname.startsWith("/painel/admin") ? " is-active" : ""}`} type="button" onClick={() => setAdminMenuOpen((open) => !open)} aria-expanded={adminMenuOpen || pathname.startsWith("/painel/admin")} aria-controls="submenu-administracao" aria-label="Abrir ou fechar subseções de Administração" title={t.admin}><SidebarIcon name="admin" /><span className="dashboard-nav__label">{t.admin}</span></button><div className={`dashboard-nav__admin-menu${adminMenuOpen || pathname.startsWith("/painel/admin") ? " is-visible" : ""}`} id="submenu-administracao" aria-label={t.adminSections}><Link className={activeClass("/painel/admin?area=operacao")} href="/painel/admin?area=operacao">{t.operation}</Link><Link className={activeClass("/painel/admin?area=conteudo")} href="/painel/admin?area=conteudo">{t.content}</Link><Link className={activeClass("/painel/admin?area=controle")} href="/painel/admin?area=controle">{t.control}</Link></div></div>}
