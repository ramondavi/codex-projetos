...
export function AssistedCatalogingWorkspace({ requestId, editable, isRaue, authorities, controlledTerms, initialPeople, initialTerms, initialCdu, initialCutter, initialCardDetails, previewBase }: {
  ...
  const [cduDescription, setCduDescription] = useState("");
  const [cduDescriptionKnown, setCduDescriptionKnown] = useState(false);
  ...
  useEffect(() => {
    let active = true;
    const code = cduCode.trim();
    if (!code) { setCduDescription(""); setCduDescriptionKnown(false); return; }
    createClient().from("cdu_code_descriptions").select("description").eq("cdu_code", code).maybeSingle().then(({ data }) => {
      if (!active) return;
      setCduDescription(data?.description ?? "");
      setCduDescriptionKnown(Boolean(data));
    });
    return () => { active = false; };
  }, [cduCode]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const [{ error }, { error: detailsError }, { error: descriptionError }] = await Promise.all([
        supabase.rpc("save_assisted_cataloging_v2", { target_request_id: requestId, payload: { people, terms, cduCode, cutterCode } }),
        supabase.rpc("save_request_card_details", { target_request_id: requestId, payload: cardDetails }),
        cduDescription.trim() ? supabase.rpc("save_cdu_code_description", { cdu_code_value: cduCode, description_value: cduDescription }) : Promise.resolve({ error: null }),
      ]);
      setSaveState(error || detailsError || descriptionError ? "error" : "saved");
    }, 800);
    return () => window.clearTimeout(timer);
  }, [people, terms, cduCode, cduDescription, cutterCode, cardDetails, requestId, editable, ready, valid]);
  ...
  const isFixedPerson = (person: PersonEntry) => ["author", "advisor", "coadvisor"].includes(person.role);
  const canDragPerson = (person: PersonEntry) => editable && (person.role === "author" ? isRaue && people.filter((item) => item.role === "author").length > 1 : !isFixedPerson(person));
  function movePerson(from: number, to: number) {
    if (from === to) return;
    const source = people[from]; const target = people[to];
    const withinAuthors = source?.role === "author" && target?.role === "author" && isRaue;
    const withinMovablePeople = source && target && !isFixedPerson(source) && !isFixedPerson(target);
    if (!withinAuthors && !withinMovablePeople) return;
    setPeople((current) => { const next = [...current]; const [moved] = next.splice(from, 1); next.splice(to, 0, moved); return next; });
  }
  ...
  return <><section className="panel assisted-cataloging">
    ...
    <fieldset><legend>Pessoas relacionadas</legend><p className="field-help">A ordem é fixa: autor(es), orientador e, quando houver, coorientador. Na banca, orientador é o 1º membro e coorientador é o 2º. Arraste apenas autores entre si no RAU+E e os demais membros da banca entre si.</p><div className="cataloging-list">{people.map((person, index) => <article className="cataloging-row cataloging-row--person" key={`${person.role}-${index}`} draggable={canDragPerson(person)} onDragStart={() => setDraggedPerson(index)} onDragOver={(event) => { if (draggedPerson !== null) event.preventDefault(); }} onDrop={() => { if (draggedPerson !== null) movePerson(draggedPerson, index); setDraggedPerson(null); }}><span className="drag-handle" aria-hidden="true">{canDragPerson(person) ? "⋮⋮" : ""}</span><label>Papel no trabalho<select value={person.role} disabled={!editable || isFixedPerson(person)} onChange={(event) => updatePerson(index, { role: event.target.value })}>{(isFixedPerson(person) ? [[person.role, roleLabels[person.role]]] : [["committee_member", roleLabels.committee_member], ["related_person", roleLabels.related_person]]).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Nome como aparece no trabalho<input value={person.transcribedName} disabled={!editable} maxLength={300} placeholder="Transcreva da folha de rosto" onChange={(event) => updatePerson(index, { transcribedName: event.target.value, authorizedName: person.authorityId ? person.authorizedName : event.target.value })} /></label><label className="autocomplete-field">Nome autorizado para a ficha<input value={person.authorizedName} disabled={!editable} maxLength={300} placeholder="Selecione ou informe a forma autorizada" autoComplete="off" onFocus={() => setActiveAutocomplete(`authority-${index}`)} onBlur={() => window.setTimeout(() => setActiveAutocomplete(null), 120)} onChange={(event) => { setActiveAutocomplete(`authority-${index}`); selectAuthority(index, event.target.value); }} />{editable && activeAutocomplete === `authority-${index}` && authorityMatches(person.authorizedName).length > 0 && <span className="cataloging-autocomplete">{authorityMatches(person.authorizedName).map((item) => <button type="button" key={item.id} onMouseDown={(event) => event.preventDefault()} onClick={() => { updatePerson(index, { authorizedName: item.authorized_name, authorityId: item.id }); setActiveAutocomplete(null); }}>{item.authorized_name}</button>)}</span>}</label>{editable && !isFixedPerson(person) && <button className="text-action" type="button" onClick={() => setPeople((current) => current.filter((_, position) => position !== index))}>Remover</button>}</article>)}</div>{editable && <button className="button button--secondary button--small" type="button" onClick={() => setPeople((current) => [...current, { authorityId: "", role: "committee_member", transcribedName: "", authorizedName: "" }])}>Adicionar membro da banca</button>}</fieldset>
