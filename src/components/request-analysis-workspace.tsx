...
function fieldIcon(key: string): AppIconName {
  if (key.includes("author") || key.includes("advisor") || key.includes("committee")) return "person";
  if (key.includes("keyword") || key.includes("subject")) return "tag";
  if (key.includes("url")) return "link";
  return "document";
}
...
export function RequestAnalysisWorkspace({ requestId, initialAnalysisNotes, initialInternalNote, initialSavedAt, initialReviewCompletedAt, initialCorrectionBaselines, editable, templates, fields }: { requestId: string; initialAnalysisNotes: string; initialInternalNote: string; initialSavedAt: string | null; initialReviewCompletedAt: string | null; initialCorrectionBaselines: Record<string, unknown>; editable: boolean; templates: IssueTemplate[]; fields: ReviewField[] }) {
  ...
  return <><section className="analysis-workspace">
    <div className="analysis-workspace__heading"><div><p className="eyebrow"><AppIcon name="review" /> Conferência do envio</p><h2>Verifique os dados informados pelo estudante</h2><p>Em cada item, escolha uma única ação: confirmar, corrigir diretamente ou pedir ajuste ao estudante.</p></div></div>
