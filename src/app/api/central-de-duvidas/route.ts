...
export async function GET(request: Request) {
  ...
  const results = [...quickDoubts, ...faqs, ...helpArticles].filter((item) => normalizeKnowledge(knowledgeText(item)).includes(term)).slice(0, 8).map(({ body: _body, sections: _sections, ...item }) => item);
