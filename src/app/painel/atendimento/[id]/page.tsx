...
export default async function RequestAnalysisPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data }, { data: templates }, { data: authorities }, { data: controlledTerms }, { data: assistedPeople }, { data: selectedTerms }, { data: catalogingMetadata }, { data: nadaConsta }, { data: repositoryProgress }, { data: publication }, { data: timeline }, { data: homologation }, { data: staff }, { data: correctionBaselines }] = await Promise.all([
  ...
  const reviewFields: ReviewField[] = [
    { key: "title", label: "Título do trabalho", value: request.title, multiline: true }, { key: "subtitle", label: "Subtítulo", value: request.subtitle ?? "" }, { key: "equivalent_title", label: "Título em outro idioma", value: request.equivalent_title ?? "" }, { key: "public_work_url", label: "Link público para a versão final", value: request.public_work_url }, ...(request.library_note?.trim() ? [{ key: "library_note", label: "Mensagem deixada pelo estudante", value: request.library_note, multiline: true }] : []),
  ];
