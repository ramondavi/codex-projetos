...
type RawDetail = {
  ...
  enrollment: { registration_number: string; program: { name: string; level: string; coordination_magic_link_enabled: boolean } | { name: string; level: string; coordination_magic_link_enabled: boolean }[] | null } | { registration_number: string; program: { name: string; level: string; coordination_magic_link_enabled: boolean } | { name: string; level: string; coordination_magic_link_enabled: boolean }[] | null }[] | null;
...
export default async function RequestAnalysisPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data }, { data: templates }, { data: authorities }, { data: controlledTerms }, { data: assistedPeople }, { data: selectedTerms }, { data: catalogingMetadata }, { data: nadaConsta }, { data: repositoryProgress }, { data: publication }, { data: timeline }, { data: homologation }] = await Promise.all([supabase.from("cataloging_requests").select(`
    ...
    enrollment:academic_enrollments!cataloging_requests_academic_enrollment_id_fkey(registration_number, program:academic_programs!academic_enrollments_academic_program_id_fkey(name, level, coordination_magic_link_enabled)),
    people:request_people(role, transcribed_name), keywords:request_keywords(language, term, position),
    card_details:request_card_details(deposit_year, defense_year, extent_unit, extent_count, has_illustrations, advisor_note_label, coadvisor_note_label),
  ...
  const cardDetails = first(request.card_details);
