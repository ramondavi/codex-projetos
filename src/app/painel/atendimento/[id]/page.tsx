...
export default async function RequestAnalysisPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="dashboard-main dashboard-main--analysis"><Link className="back-link" href="/painel/fila">← Voltar para a fila</Link><header className="analysis-case-header"><div className="analysis-case-header__top"><p className="eyebrow">Atendimento</p><div className="analysis-case-header__actions"><span className={`request-status ${statusClass}`}>{attendanceStatus}</span><a className="button button--primary button--small" href={request.public_work_url} target="_blank" rel="noreferrer">Abrir trabalho completo ↗</a></div></div><div className="analysis-case-header__protocol"><ProtocolCopyButton protocol={request.protocol} /> <time title={new Intl.DateTimeFormat("pt-BR", { dateStyle: "medium", timeStyle: "short" }).format(new Date(request.submitted_at))}>· aberto há {Math.max(0, Math.floor((Date.now() - new Date(request.submitted_at).getTime()) / 86400000))} dias</time></div><h1>{request.title}</h1><dl className="analysis-case-header__details"><div><dt>Estudante</dt><dd>{first(student?.profile)?.full_name ?? "Estudante"}</dd></div><div><dt>Curso</dt><dd>{program?.name ?? "Programa não identificado"}</dd></div></dl></header>
