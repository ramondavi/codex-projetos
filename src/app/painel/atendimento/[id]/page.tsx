...
type RawDetail = {
  id: string; protocol: string; status: string; title: string; subtitle: string | null; equivalent_title: string | null; public_work_url: string; library_note: string | null; assigned_to: string | null;
...
export default async function RequestAnalysisPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const reviewFields: ReviewField[] = [
    { key: "registration_number", label: "Matrícula", value: enrollment?.registration_number ?? "" }, { key: "title", label: "Título", value: request.title, multiline: true }, { key: "subtitle", label: "Subtítulo", value: request.subtitle ?? "" }, { key: "equivalent_title", label: "Título equivalente", value: request.equivalent_title ?? "" }, { key: "author", label: "Autor", value: personValue("author") }, { key: "advisor", label: "Orientador", value: personValue("advisor") }, { key: "coadvisor", label: "Coorientador", value: personValue("coadvisor") }, { key: "keywords_pt", label: "Palavras-chave em português", value: ptKeywords.map((item) => item.term).join("\n"), multiline: true }, { key: "keywords_en", label: "Palavras-chave em inglês", value: enKeywords.map((item) => item.term).join("\n"), multiline: true }, { key: "public_work_url", label: "Link público do trabalho", value: request.public_work_url }, { key: "library_note", label: "Observação do estudante", value: request.library_note ?? "", multiline: true },
  ];
  ...
  return <main className="dashboard-main dashboard-main--analysis"><Link className="back-link" href="/painel/fila">← Voltar para a fila</Link><div className="page-heading analysis-heading"><div><p className="eyebrow">Atendimento {request.protocol}</p><h1>{request.title}</h1><p>{first(student?.profile)?.full_name ?? "Estudante"} · {program?.name ?? "Programa"}</p></div><span className="request-status">{request.status === "completed" ? "Protocolo encerrado" : editable ? "Em edição por você" : ownsTicket ? "Atendimento atribuído a você" : `Bloqueado por ${first(request.assignee)?.full_name ?? "outro profissional"}`}</span></div>
    <a className="button button--secondary button--small" href={request.public_work_url} target="_blank" rel="noreferrer">Abrir trabalho completo ↗</a>
    <RequestAnalysisWorkspace requestId={request.id} initialAnalysisNotes={analysis?.analysis_notes ?? ""} initialInternalNote={analysis?.internal_note ?? ""} editable={editable} templates={templates ?? []} fields={reviewFields} />
    <AssistedCatalogingWorkspace requestId={request.id} editable={editable} authorities={authorities ?? []} controlledTerms={controlledTerms ?? []} initialPeople={initialPeople} initialTerms={initialTerms} initialCdu={catalogingMetadata?.cdu_code ?? ""} initialCutter={catalogingMetadata?.cutter_code ?? ""} initialCardDetails={{ depositYear: cardDetails?.deposit_year ?? 0, defenseYear: cardDetails?.defense_year ?? 0, extentUnit: cardDetails?.extent_unit ?? "pages", extentCount: cardDetails?.extent_count ?? 0, hasIllustrations: cardDetails?.has_illustrations ?? false, advisorNoteLabel: cardDetails?.advisor_note_label ?? "Orientador", coadvisorNoteLabel: cardDetails?.coadvisor_note_label ?? "Coorientador" }} previewBase={previewBase} />
    <NadaConstaReview document={nadaConsta} editable={ownsTicket && request.status === "approved"} />{request.status === "approved" && ownsTicket && <ProtocolClosure requestId={id} coordinationEnabled={program?.coordination_magic_link_enabled ?? false} ready={ready} />}{publication && <section className="panel publication-record"><p className="eyebrow">Publicação verificada</p><h2>Protocolo encerrado</h2><a href={publication.permanent_url} target="_blank" rel="noreferrer">{publication.permanent_url} ↗</a><p>O Nada Consta entrou na contagem de 60 dias para expurgo.</p></section>}<RequestTimeline events={(timeline ?? []) as TimelineEvent[]} />
  </main>;
}
