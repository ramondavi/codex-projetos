...
export default async function CatalogingCardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const workNature = program?.work_type === "thesis" ? "Tese" : program?.work_type === "dissertation" ? "Dissertação" : "Trabalho de Conclusão de Curso";
  ...
  const ready = Boolean(metadata?.cdu_code && metadata?.cutter_code && details?.deposit_year && details?.defense_year && details?.extent_count && staff?.professional_name && staff?.crb && people?.some((person) => person.role === "author") && people?.some((person) => person.role === "advisor") && subjects?.some((subject) => subject.is_primary));
