...
export default async function HelpPage() {
  ...
  return <><SiteHeader /><main className="faq-page">
    ...
    <section className="container knowledge-category" aria-labelledby="artigos-ajuda"><p className="eyebrow">{t.articles}</p><h2 id="artigos-ajuda">{t.articleTitle}</h2><div className="knowledge-cards">{articles.map((article) => <article key={article.id}><AppIcon name={article.category === "Análise" ? "review" : article.category === "Autodepósito" ? "upload" : "document"} /><span>{helpCategoryLabel(language, article.category)}</span><h3>{article.title}</h3><p>{article.summary}</p><Link href={`/ajuda/artigos/${article.slug}`}>{t.read}</Link></article>)}</div></section>
    <section className="container help-contact" aria-labelledby="contato-ajuda"><div><p className="eyebrow">{t.contact}</p><h2 id="contato-ajuda">{t.contactTitle}</h2><p>{t.contactIntro}</p></div><address><a href="mailto:bibarq@ufba.br">bibarq@ufba.br</a><a href="tel:+557132835888">(71) 3283-5888</a></address></section>
  </main></>;
}
