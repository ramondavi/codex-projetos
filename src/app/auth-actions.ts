...
import { isUfbaEmail, normalizeEmail, normalizedSignupMetadata, validateSignup } from "@/domain/auth/account";
...
export async function requestEmailChange(formData: FormData) {
  const email = normalizeEmail(String(formData.get("email") ?? ""));
  if (!isUfbaEmail(email)) redirect(destination("/painel/conta", "error", "Use um novo endereço institucional @ufba.br."));
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/entrar");
  if (normalizeEmail(user.email ?? "") === email) redirect(destination("/painel/conta", "error", "Informe um endereço diferente do atual."));
  const { error } = await supabase.auth.updateUser(
    { email },
    { emailRedirectTo: `${await siteOrigin()}/auth/callback?next=/painel/conta` },
  );
  if (error) redirect(destination("/painel/conta", "error", "Não foi possível iniciar a alteração do e-mail."));
  redirect(destination("/painel/conta", "message", "Solicitação enviada. Confirme a alteração nos endereços indicados pelo Supabase."));
}
